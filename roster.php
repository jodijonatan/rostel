...
if (isset($_GET['hapus'])) {
  $id = $_GET['hapus'];
  $conn->query("DELETE FROM roster WHERE id_roster=$id");
  header("Location: roster.php");
  exit;
}
...
if (isset($_GET['edit'])) {
  $editMode = true;
  $id_edit = $_GET['edit'];
  $editData = $conn->query("SELECT * FROM roster WHERE id_roster=$id_edit")->fetch_assoc();
}
...
if (isset($_POST['update'])) {
  ...
  $cek = $conn->query("SELECT * FROM roster WHERE id_guru=$id_guru AND hari='$hari' AND id_roster!=$id AND NOT (jam_selesai<='$jam_mulai' OR jam_mulai>='$jam_selesai')");
  if ($cek->num_rows > 0) {
    $error = "Guru sudah memiliki jadwal di waktu ini!";
  } else {
    $stmt = $conn->prepare("UPDATE roster SET id_guru=?, id_mapel=?, id_kelas=?, hari=?, jam_mulai=?, jam_selesai=? WHERE id_roster=?");
    $stmt->bind_param("iiisssi", $id_guru, $id_mapel, $id_kelas, $hari, $jam_mulai, $jam_selesai, $id);
    $stmt->execute();
    header("Location: roster.php");
    exit;
  }
...
// Ambil data roster
$roster = $conn->query("
  SELECT r.id_roster, g.nama AS guru, m.nama AS mapel, k.nama_kelas, r.hari, r.jam_mulai, r.jam_selesai
  FROM roster r
    ...
    <div class="row g-2">
      <div class="col-md-2">
        <select name="id_guru" class="form-control" required>
          ...
          while ($g = $guruList->fetch_assoc()): ?>
            <option value="<?= $g['id_guru'] ?>" <?= $editMode && $editData['id_guru'] == $g['id_guru'] ? 'selected' : '' ?>>
              <?= $g['nama'] ?>
            </option>
          <?php endwhile; ?>
        </select>
      ...
      <div class="col-md-2">
        <select name="id_mapel" class="form-control" required>
          ...
          while ($m = $mapelList->fetch_assoc()): ?>
            <option value="<?= $m['id_mapel'] ?>" <?= $editMode && $editData['id_mapel'] == $m['id_mapel'] ? 'selected' : '' ?>>
              <?= $m['nama'] ?>
            </option>
          <?php endwhile; ?>
        </select>
      ...
      <div class="col-md-2">
        <select name="id_kelas" class="form-control" required>
          ...
          while ($k = $kelasList->fetch_assoc()): ?>
            <option value="<?= $k['id_kelas'] ?>" <?= $editMode && $editData['id_kelas'] == $k['id_kelas'] ? 'selected' : '' ?>>
              <?= $k['nama_kelas'] ?>
            </option>
          <?php endwhile; ?>
        </select>
      ...
      <div class="col-md-2">
        <select name="hari" class="form-control" required>
          ...
          ?>
            <option value="<?= $h ?>" <?= $editMode && $editData['hari'] == $h ? 'selected' : '' ?>><?= $h ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="col-md-2">
        <select name="les" class="form-control" required>
          ...
          <?php foreach ($lesList as $les => $jam): ?>
            <option value="<?= $les ?>" <?= ($editMode && $editData['jam_mulai'] == $jam[0]) ? 'selected' : '' ?>>
              <?= $les ?> (<?= $jam[0] ?> - <?= $jam[1] ?>)
            </option>
          <?php endforeach; ?>
        </select>
      ...
      <div class="col-md-2">
        <?php if ($editMode): ?>
          <input type="hidden" name="id_roster" value="<?= $editData['id_roster'] ?>">
          <button type="submit" name="update" class="btn btn-success w-100">Update</button>
          <a href="roster.php" class="btn btn-secondary w-100 mt-2">Batal</a>
        <?php else: ?>
      ...
      <tbody>
        <?php while ($r = $roster->fetch_assoc()): ?>
          <tr>
            ...
            <td>
              <a href="roster.php?edit=<?= $r['id_roster'] ?>" class="btn btn-warning btn-sm">Edit</a>
              <a href="roster.php?hapus=<?= $r['id_roster'] ?>"
